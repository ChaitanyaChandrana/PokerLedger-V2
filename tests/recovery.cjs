...
(async()=>{
 ...
 setup({guestName:'Chaitanya'});assert.equal(state.jName,'Chaitanya');assert.equal(state.cHostName,'Chaitanya');assert(text(tree).includes('Welcome backChaitanya'));assert.equal(writes,0);assert.equal(state.screen,'join');
 assert(!nodes().some(n=>n.props.id==='setup-jCode'));assert(!nodes().some(n=>n.props.id==='setup-jName'));const codeEdit=nodes().find(n=>n.type==='button'&&text(n)==='Change code');assert(codeEdit);codeEdit.props.onClick();render();assert(nodes().some(n=>n.props.id==='setup-jCode'));nodes().find(n=>n.type==='button'&&text(n)==='Done').props.onClick();render();assert(!nodes().some(n=>n.props.id==='setup-jCode'));assert(nodes().some(n=>n.type==='details'&&n.props.className==='quick-join-optional'&&!n.props.open));
 const change=nodes().find(n=>n.type==='button'&&text(n)==='Change name');assert(change);change.props.onClick();render();assert.equal(state.jName,'');assert.equal(store.has('pokerGuestName'),false);assert(nodes().some(n=>n.props.id==='setup-jName'));assert.equal(auth.currentUser.uid,'uh');assert.equal(writes,0);
